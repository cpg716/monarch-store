...
- **Fix (current):** Command is written to a temp file (`/tmp/monarch-cmd-<ts>.json`); only the **file path** is passed to the helper. Helper reads JSON from file, parses it, then deletes the file. This avoids argv length and escaping issues.
...
- **Flow:** `update.rs` → `invoke_helper(HelperCommand::Sysupgrade)` → same temp-file + pkexec as install.
...
| `src-tauri/monarch-gui/src/commands/update.rs` | `perform_system_update` → `invoke_helper(Sysupgrade)`. |
...
4. **System update:** “Update All” runs Sysupgrade via helper, no freeze, success or clear error.
